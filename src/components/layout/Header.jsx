  ...
  return (
    <header
      className={`w-full fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-gray-900 shadow-md py-3 xl:py-4' : 'bg-gray-900 py-5 xl:py-6'
      }`}
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center">
            <div className="bg-gradient-to-r from-blue-700 to-indigo-900 w-10 h-10 sm:w-12 sm:h-12 xl:w-14 xl:h-14 rounded-md flex items-center justify-center shadow-md">
              <span className="text-white text-xl sm:text-2xl xl:text-3xl font-bold">CD</span>
            </div>
            <div className="ml-3 xl:ml-4">
              <h1 className="text-xl sm:text-2xl xl:text-3xl font-bold text-white">campus</h1>
              <p className="text-sm xl:text-base -mt-1 text-blue-300 ">Dost</p>
            </div>
          </Link>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`px-4 py-2 xl:px-5 xl:py-3 mx-1 rounded-md font-medium text-base xl:text-lg ${
                  isActive(item.path) ? 'bg-white/10 text-white' : 'text-white hover:bg-white/10'
